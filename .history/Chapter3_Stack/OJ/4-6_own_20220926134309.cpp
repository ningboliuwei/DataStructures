...
main() {
    ...
    for (int i = 0; i <= index; i++) {
        char currentCharacter = suffixExpression[i];

        if (currentChar == '0' || currentChar == '1' || currentChar == '2' || currentChar == '3' || currentChar == '4'
            || currentChar == '5' || currentChar == '6' || currentChar == '7' || currentChar == '8'
            || currentChar == '9') {
            Push(calculationStack, currentChar);
        } else if ((currentChar == '+' || currentChar == '-' || currentChar == '*' || currentChar == '/')) {
            char stackTopElement;
            Pop(calculationStack, stackTopElement);
            int x = (int)stackTopElement - '0';
            Pop(calculationStack, stackTopElement);
            int y = (int)stackTopElement - '0';
            int result = 0;
