...
int InsertList(SeqList *list, int i, DataType x) {
    ...
    if (list->last == MAX_COLUMN_COUNT - 1) { return 0; }

    if (i < 0 || i > list->last + 1) { return 0; }

    for (index = list->last + 1; index >= i; index--) { list->data[index] = list->data[index - 1]; }
...
int ClearStack(LinkStack *stack) {
    ...
    while (!IsEmptyStack(stack)) { Pop(stack, topElement); }

    return 1;
}

int ShowLines(SeqList *lines[MAX_LINE_COUNT]) {
    for (int i = 0; i < MAX_LINE_COUNT; i++) {
        for (int j = 0; j <= lines[i]->last; j++) {
            char currentChar = lines[i]->data[j];
            if (currentChar == '$') {
                return 1;
            } else {
...
void InputLines(SeqList *lines[MAX_LINE_COUNT]) {
    int lineIndex = 0;
    ...
    while (scanf("%c", &currentChar)) {
        if (currentChar == '\n' || currentChar == '$') {
            ...
            while (!IsEmptyStack(lineStack)) {
                ...
                // 头插，这样就不用考虑倒序的问题
                InsertList(lines[lineIndex], 0, topElement);
            }
            lineIndex++;
        } else if (currentChar == '#') {
            Pop(lineStack, topElement);
...
void InitializeLines(SeqList *lines[MAX_LINE_COUNT]) {
    for (int i = 0; i < MAX_LINE_COUNT; i++) {
        lines[i] = CreatList();
        for (int j = 0; j < MAX_COLUMN_COUNT; j++) { lines[i]->data[j] = '\0'; }
    }
}
