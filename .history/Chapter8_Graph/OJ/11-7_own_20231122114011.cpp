...
void MiniSpanTree_KRUSKAL(MGraph G, int n, int v) {
    ...
    while (foundCount < G.n - 1) {
        ...
        int startVexIndex = edgeFlags[minEdgeIndex].u;
        // 选中的这条权值最小边的终点下标保存到 pos2 中
        int endVexIndex = edgeFlags[minEdgeIndex].v;
        // 如果找到的权值最小边的起点与终点不属于同一个集合（连通分量），
        if (vexSets[startVexIndex] != vexSets[endVexIndex]) {
            // 那么将这条边的 flag 设为 1（表示选中了该条边作为最小生成树中的一条边）
            edgeFlags[minEdgeIndex].flag = 1;
            // 选中的边数 + 1
            foundCount++;
            // 权值最小边的起点所属的集合保存到 startSet1 中
            int set1 = vexSets[startVexIndex];
            // 权值最小边的终点所属的集合保存到 endSet 中
            int endSet = vexSets[endVexIndex];
