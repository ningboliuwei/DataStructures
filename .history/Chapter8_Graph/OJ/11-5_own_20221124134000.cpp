...
void ShortestPath_DIJ(MGraph G, int start) {
    // P[] 表示从源点（如
    // v0）到各顶点的最短路径上，此顶点的前一顶点的下标；若从起点到该顶点无路径，则设值为
    // -1；本数组用于从某个顶点开始，倒推到起点的每一条边（因为一条最短路径都是由若干条最短路径的“前缀”组成的），也就是说，往前递归地寻找离当前这个顶点最近的点
    int P[MAXVER];
    // 《D》可以理解为《DISTANCE》
    ...
    for (i = 0; i < G.vexnum; ++i) {
        ...
        for (w = 0; w < G.vexnum; ++w)
            // 逐渐更新 D[] 及 P[]
            if (!final[w]) {
                if (D[w] + G.adjlist[v][w] < D[w]) {
                    D[w] = min + G.adjlist[v][w];
                    P[w] = v;
                }
            }
    }
    // 依次输出所有最短路径，具体思路看 P[] 的注释
    for (w = 0; w < G.vexnum; ++w) {
        if (w != start) {
            // 输出当前顶点
            printf("%d", w);
