...
void MiniSpanTree_KRUSKAL(MGraph G, int n, int v) {
    // 定义一个名为 vexFlags
    // 的数组，用于给每一个顶点设置其所属的集合，可以看成是这个顶点所属的联通分量的编号，用一个整数来表示。初始时，每个顶点分属不同的集合（联通分量）
    int vexFlags[MAX_VEX_COUNT];
    // 定义一个名为 edgeFlags 的数组，用于给每条边设置一个标记，其中 0 表示尚未被选中，1 表示被选中，2 表示被淘汰
