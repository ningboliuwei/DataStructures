...
void MiniSpanTree_KRUSKAL(MGraph G, int n, int v) {
        ...
        for (int j = 0; j < G.n; j++) {
            for (int k = j; k < G.n; k++) {
                if (G.edges[j][k] > 0) {
                    edgeFlags[i].u = j;
                    edgeFlags[i].v = k;
                    edgeFlags[i].weight = G.edges[j][k];
                    edgeFlags[i].flag = 0;
                }
            }
        }
    ...
    int min = INFINITY;
    // 找出所有边里权值最小，且 flag 为 0 的那条边
    int minEdgePos = 0;
    for (int i = 0; i < G.e; i++) {
        if (min < edgeFlags[i].weight && edgeFlags[i].flag == 0) {
            min = edgeFlags[i].weight;
            minEdgePos = i;
        }
    }
    if (vexSets[edgeFlags[minEdgePos].u] != vexSets[edgeFlags[minEdgePos].v]) {
