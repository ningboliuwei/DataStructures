...
node *createlink() {                           // 创建链表
    ...
    while (scanf("%d", &x) && x != -1) {        // 输入链表元素，以 -1 结束
        ...
        if (tail == head || x >= tail->data) {     // 如果链表为空或者 x 大于等于尾节点的数据域
            tail->next = p;                        // 将尾节点的 next 指针指向新节点
            tail = p;                              // 将尾指针指向新节点
        } else {                                   // 如果 x 小于尾节点的数据域
            node *q = head;                        // 定义指针 q，初始指向头节点
            while (q->next && x > q->next->data) { // 遍历链表，查找插入位置
                q = q->next;                       // 将 q 指向下一个节点
            }
            p->next = q->next; // 将新节点的 next 指针指向 q 的下一个节点
            q->next = p;       // 将 q 的 next 指针指向新节点
        }
    }
