...
int main() {
    ...
    while (choice != 4) {
        if (choice == 1) {
            ...
            int result = InQueue(queues[department - 1], registerNumber);

            if (result == 1) {
                printf("挂号成功请等候，您前面排队人数%d\n", QueueLength(queues[department - 1]) - 1);
            }
        } // 排队
        else if (choice == 2) {
            ShowDepartmentChooseMenu();
            ...
            if (QueueEmpty(queues[department - 1])) {
                printf("该诊室尚无人排队\n");
            } else {
                int result = OutQueue(queues[department - 1], registerNumber);
            ...
            scanf("%d", &department);
            // 注意这里要减 1 ！
            ShowQueueInfo(queues[department - 1]);
        } // 查看排队
        else {}
