...
void PreOrderTraverse(HTNode treeNodes[], int index, int text) {
    if (treeNodes[index].leftChild == -1 && treeNodes[index].rightChild == -1 && treeNodes[index].text == text) {
        printf("--%c\n", treeNodes[index].text);
    }

    if (treeNodes[index].leftChild != -1) {
        printf("0");
        PreOrderTraverse(treeNodes, treeNodes[index].leftChild, text);
    }

    if (treeNodes[index].rightChild != -1) {
        printf("1");
        PreOrderTraverse(treeNodes, treeNodes[index].rightChild，text);
    }
}

void Encoding(HTNode treeNodes[], int treeNodeCount, textCode textCodeList[]) {
    int index = treeNodeCount - 1;
    PreOrderTraverse(treeNodes, index);
}
