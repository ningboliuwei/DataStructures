...
// 根据编码输出单个字符
void ShowTextByCode(HTNode treeNodes[], int treeNodeCount, char code[]) {
    bool match = true;
...
// 译码（根据编码输出字符串）
void DecodeText(HTNode treeNodes[], int treeNodeCount, char code[]) {
    char code[100];
    scanf("%s", code);

    for (int i = 0; i < strlen(code); i++) {
        ShowTextByCode(treeNodes, treeNodeCount, code);
    }
}
