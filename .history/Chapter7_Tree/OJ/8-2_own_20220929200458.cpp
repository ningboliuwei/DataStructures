...
int create(BiTree &T, char pa[], int low_a, int high_a, char pb[], int low_b,
    ...
    if (leftChildTreeNodeCount == 0) {
        ...
        create(T->lchild, pa, low_a + leftChildTreeNodeCount + 1, high_a, pb, low_b, root_pos_b - 1);
    }
    // 开始右子树处的递归
    if (rightChildTreeNodeCount == 0) {
        T->rchild = NULL;
    } else {
        create(T->rchild, pa, high_a - 1, low_a + leftChildTreeNodeCount, pb, low_b, root_pos_b - 1);
    }
