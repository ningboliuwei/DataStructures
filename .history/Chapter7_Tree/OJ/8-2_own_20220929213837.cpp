...
int create(BiTree &T, char pa[], int low_a, int high_a, char pb[], int low_b,
    ...
    int root_pos_b = high_b;
    char root = pb[root_pos_b];
    T->data = root;
    int root_pos_a = -1;
    ...
    int leftChildTreeNodeCount = root_pos_a - low_a;
    int rightChildTreeNodeCount = high_a - root_pos_a;

    // 开始左子树处的递归
    if (leftChildTreeNodeCount == 0) {
        T->lchild = NULL;
    } else {
        create(T->lchild, pa, low_a, root_pos_a - 1, pb, low_b, high_b - rightChildTreeNodeCount - 1);
    }
    // 开始右子树处的递归
    if (rightChildTreeNodeCount == 0) {
        T->rchild = NULL;
    } else {
        create(T->rchild, pa, root_pos_b + 1, high_a, pb, high_b - rightChildTreeNodeCount, high_b - 1);
    }
