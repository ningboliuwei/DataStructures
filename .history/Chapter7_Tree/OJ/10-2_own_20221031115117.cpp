...
void ShowTextByCode(HTNode treeNodes[], int treeNodeCount, char code[], int &codeLength) {
    for (int i = 0; i < treeNodeCount; i++) {
        ...
        if (match) {
            codeLength = strlen(treeNodes[i].code);
...
void DecodeText(HTNode treeNodes[], int treeNodeCount) {
    ...
    for (int i = 0; i < strlen(encodedText); i++) {
        int codeLength;
        ShowTextByCode(treeNodes, treeNodeCount, encodedText, codeLength);
        i += codeLength;
    }
}
