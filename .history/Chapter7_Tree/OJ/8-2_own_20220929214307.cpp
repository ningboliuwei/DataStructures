...
int create(BiTree &T, char pa[], int low_b, int high_b, char pb[], int low_c,
    ...
    if (leftChildTreeNodeCount == 0) {
        ...
        create(T->lchild, pa, low_b, root_pos_a - 1, pb, low_b, high_b - rightChildTreeNodeCount - 1);
    }
    // 开始右子树处的递归
    if (rightChildTreeNodeCount == 0) {
        T->rchild = NULL;
    } else {
        create(T->rchild, pa, root_pos_a + 1, high_b, pb, high_b - rightChildTreeNodeCount, high_b - 1);
    }
