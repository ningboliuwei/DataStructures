...
void Encode(HTNode treeNodes[], int index, int text, int treeNodeCount) {
    int startIndex = -1;
    for (int startIndex = 0; startIndex < treeNodeCount; startIndex++) {
        if (treeNodes[startIndex].text == text) {
            break;
        }
    }
}
...
void ShowCoding(HTNode treeNodes[], int treeNodeCount, textCode textCodeList[]) {
    for (int i = 0; i < 7; i++) {
        ...
        FindPath(treeNodes, index, treeNodes[i].text);
    }
}
