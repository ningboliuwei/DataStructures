...
int create(BiTree &T, char pb[], int low_b, int high_b, char pc[], int low_c,
    ...
    if (leftChildTreeNodeCount == 0) {
        ...
        create(T->lchild, pb, low_b, root_pos_b - 1, pc, low_c, high_c - rightChildTreeNodeCount - 1);
    }
    // 开始右子树处的递归
    if (rightChildTreeNodeCount == 0) {
        T->rchild = NULL;
    } else {
        create(T->rchild, pb, root_pos_b + 1, high_b, pc, high_c - rightChildTreeNodeCount, high_c - 1);
    }
