...
void Encoding(HTNode treeNodes[], int treeNodeCount, textCode textCodeTable[MAX_NODE_COUNT]) {
    int depth = floor(log(treeNodeCount) / log(2));
    char *code = (char *)malloc(sizeof(char *) * depth);

    for (int i = 0; i < treeNodeCount; i++) {
        int currentNodeIndex = i;
        int pos = 0;

        while (treeNodes[currentNodeIndex].parent.parent != -1) {
            if (treeNodeCount[currentNodeIndex].leftChild == currentNodeIndex) {
                code[pos] = 0;
            }

            if (treeNodeCount[currentNodeIndex].rightChild == currentNodeIndex) {
                code[pos] = 1;
            }
