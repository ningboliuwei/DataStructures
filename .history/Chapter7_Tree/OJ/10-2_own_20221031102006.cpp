...
void GetCodeTable(HTNode treeNodes[], int index, char text, int treeNodeCount) {
    for (int i = 0; i < treeNodeCount; i++) {
...
void ShowCoding(HTNode treeNodes[], int treeNodeCount, int leafNodeCount) {
    for (int i = 0; i < leafNodeCount; i++) {
        GetCodeTable(treeNodes, 0, treeNodes[i].text, treeNodeCount);
        // 以下代码用于输出码表
...
int main() {
    ...
    ShowCoding(treeNodes, treeNodeCount, leafNodeCount);
    getchar();
    getchar();
}
