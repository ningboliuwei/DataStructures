...
int create(BiTree &T, char pa[], int low_a, int high_a, char pb[], int low_b,
    ...
    if (leftChildTreeNodeCount == 0) {
        // 若当前左子树元素个数为 0，则将当前元素的左孩子（T->lchild）设为 NULL
        T->lchild = NULL;
    } else {
    ...
    if (rightChildTreeNodeCount == 0) {
        // 若当前右子树元素个数为 0，则将当前元素的右孩子（T->rchild）设为 NULL
        T->rchild = NULL;
    } else {
        ...
        // 实参 pb 为中序序列
        // 右子树在中序序列中的元素范围下界为当前子树在中序序列中的根节点位置的前一个元素的后一个的下标（root_pos_b +
        // 1） 右子树在中序序列中的元素范围下界为当前子树在中序序列中的元素范围上界的下标（high_b）
