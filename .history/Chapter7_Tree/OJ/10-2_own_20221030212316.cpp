...
void PreOrderTraverse(HTNode treeNodes[], int index) {
    if (treeNodes[index].leftChild == -1 && treeNodes[index].rightChild == -1) {
        pritnf("%c", treeNodes[index].text);
    }
    printf("%c ", treeNodes[index].text);

    if (treeNodes[index].leftChild != -1) {
        PreOrderTraverse(treeNodes, treeNodes[index].leftChild);
    }

    if (treeNodes[index].rightChild != -1) {
        PreOrderTraverse(treeNodes, treeNodes[index].rightChild);
    }
}
