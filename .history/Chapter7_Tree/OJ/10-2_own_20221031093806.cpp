...
void Encode(HTNode treeNodes[], int index, char text, int treeNodeCount) {
    ...
    int pos = 0;
    treeNodes[index].code = (char *)malloc(sizeof(char) * (depth + 1));
    // 反向获取编码
    while (!IsEmptyStack(codeStack)) {
        char codeSegment;
        Pop(codeStack, codeSegment);
        treeNodes[index].code[pos] = codeSegment;
        pos++;
    }

    treeNodes[index].code[pos] = '\n';
}

void ShowCoding(HTNode treeNodes[], int treeNodeCount, textCode textCodeList[], int leafNodeCount) {
    for (int i = 0; i < leafNodeCount; i++) {
        Encode(treeNodes, 0, treeNodes[i].text, treeNodeCount);

        if (treeNodes[i].leftChild == -1 && treeNodes[i].rightChild == -1) {
            int pos = 0;
            while (treeNodes[i].code[pos] != '\n') {
...
int main() {
    ...
    ShowCoding(treeNodes, treeNodeCount, textCodeList, leafNodeCount);
    getchar();
    getchar();
}
